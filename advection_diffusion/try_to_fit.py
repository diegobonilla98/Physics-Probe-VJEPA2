...
eq_to_id = {"diffusion": 0, "advection": 1}
id_to_eq = {v: k for k, v in eq_to_id.items()}
...
def stratified_split(entries, test_frac, local_rng):
    ...
    test_ids = []
    train_ids = []
    for eq_name, idxs in by_eq.items():
...
def prepare_label_arrays(entries_list):
    y_eq = np.array([eq_to_id[e["equation"]] for e in entries_list], dtype=np.int64)
    y_params = np.array([[e["vx"], e["vy"], e["alpha"]] for e in entries_list], dtype=np.float32)
    return y_eq, y_params


def fit_koopman(train_entries, test_entries, latent_dim):
    ...
    def eval_rollout(entries_list):
        cos_by_sim = []
        mse_by_sim = []
        by_eq = {name: [] for name in eq_to_id.keys()}
        for e in entries_list:
            ...
            cos = cosine_mean_np(e_roll[1:], e["per_frame"][1:])
            mse = float(np.mean((e_roll[1:] - e["per_frame"][1:]) ** 2))
            cos_by_sim.append(cos)
            mse_by_sim.append(mse)
            by_eq[e["equation"]].append(cos)

        out = {
            "rollout_cos_mean": float(np.mean(cos_by_sim)),
            "rollout_cos_std": float(np.std(cos_by_sim)),
            "rollout_mse_mean": float(np.mean(mse_by_sim)),
        }
        for eq_name in eq_to_id.keys():
            vals = by_eq[eq_name]
            if len(vals) > 0:
                out[f"rollout_cos_mean_{eq_name}"] = float(np.mean(vals))
                out[f"rollout_cos_std_{eq_name}"] = float(np.std(vals))
            else:
                out[f"rollout_cos_mean_{eq_name}"] = None
                out[f"rollout_cos_std_{eq_name}"] = None
        return out
...
entries = []
for sim_dir in tqdm(sim_dirs, desc="Loading advection/diffusion sims"):
...
y_train_eq, y_train_params = prepare_label_arrays(train_entries)
y_test_eq, y_test_params = prepare_label_arrays(test_entries)

# Equation classification probes.
...
# Unified parameter regression [vx, vy, alpha].
...
train_adv = [e for e in train_entries if e["equation"] == "advection"]
test_adv = [e for e in test_entries if e["equation"] == "advection"]
train_diff = [e for e in train_entries if e["equation"] == "diffusion"]
test_diff = [e for e in test_entries if e["equation"] == "diffusion"]

X_adv_train = np.stack([e["per_frame"].mean(axis=0) for e in train_adv], axis=0).astype(np.float32)
X_adv_test = np.stack([e["per_frame"].mean(axis=0) for e in test_adv], axis=0).astype(np.float32)
y_adv_train = np.array([[e["vx"], e["vy"]] for e in train_adv], dtype=np.float32)
y_adv_test = np.array([[e["vx"], e["vy"]] for e in test_adv], dtype=np.float32)
pred_adv_train, pred_adv_test = fit_ridge(X_adv_train, y_adv_train, X_adv_test, ridge_lambda)

adv_vx_r2_test = r2_score_np(y_adv_test[:, 0], pred_adv_test[:, 0])
adv_vy_r2_test = r2_score_np(y_adv_test[:, 1], pred_adv_test[:, 1])
adv_vx_mae_test = mae_np(y_adv_test[:, 0], pred_adv_test[:, 0])
adv_vy_mae_test = mae_np(y_adv_test[:, 1], pred_adv_test[:, 1])
...
koopman_shared = fit_koopman(train_entries, test_entries, koopman_latent_dim)
koopman_by_eq = {}
for eq_name in eq_to_id.keys():
    tr = [e for e in train_entries if e["equation"] == eq_name]
    te = [e for e in test_entries if e["equation"] == eq_name]
    if len(tr) >= 3 and len(te) >= 2:
        koopman_by_eq[eq_name] = fit_koopman(tr, te, koopman_latent_dim)
...
metrics = {
    ...
    "parameter_regression": {
        ...
        "advection_vx_r2_test": float(adv_vx_r2_test),
        "advection_vy_r2_test": float(adv_vy_r2_test),
        "advection_vx_mae_test": float(adv_vx_mae_test),
        "advection_vy_mae_test": float(adv_vy_mae_test),
        "diffusion_alpha_r2_test": float(diff_alpha_r2_test),
        "diffusion_alpha_mae_test": float(diff_alpha_mae_test),
    },
    "koopman_shared": koopman_shared,
    "koopman_by_equation": koopman_by_eq,
}
...
summary_lines = [
    "advection vs diffusion probe summary",
    "=" * 72,
    ...
    f"advection vx R2={adv_vx_r2_test:.4f} vy R2={adv_vy_r2_test:.4f}",
    f"advection vx MAE={adv_vx_mae_test:.4f} vy MAE={adv_vy_mae_test:.4f}",
    f"diffusion alpha R2={diff_alpha_r2_test:.4f} MAE={diff_alpha_mae_test:.4f}",
    "",
    "koopman shared",
    "-" * 72,
    f"spectral_radius={koopman_shared['spectral_radius']:.6f}",
    (
        "rollout_cos test overall="
        f"{koopman_shared['test']['rollout_cos_mean']:.4f} +/- {koopman_shared['test']['rollout_cos_std']:.4f}"
    ),
    f"rollout_cos test diffusion={koopman_shared['test']['rollout_cos_mean_diffusion']:.4f}",
    f"rollout_cos test advection={koopman_shared['test']['rollout_cos_mean_advection']:.4f}",
]

if "diffusion" in koopman_by_eq:
    summary_lines.append(
        "koopman diffusion-only test cos="
        f"{koopman_by_eq['diffusion']['test']['rollout_cos_mean']:.4f} +/- "
        f"{koopman_by_eq['diffusion']['test']['rollout_cos_std']:.4f}"
    )
if "advection" in koopman_by_eq:
    summary_lines.append(
        "koopman advection-only test cos="
        f"{koopman_by_eq['advection']['test']['rollout_cos_mean']:.4f} +/- "
        f"{koopman_by_eq['advection']['test']['rollout_cos_std']:.4f}"
    )
...
axes[0, 0].hist(score_test_sim[y_test_eq == 1], bins=12, alpha=0.7, label="advection")
...
axes[0, 1].scatter(y_adv_test[:, 0], pred_adv_test[:, 0], s=24, label="vx")
axes[0, 1].scatter(y_adv_test[:, 1], pred_adv_test[:, 1], s=24, label="vy")
axes[0, 1].set_title("Advection parameter probe (test)")
...
bar_names = ["shared all", "shared diff", "shared adv"]
bar_vals = [
    koopman_shared["test"]["rollout_cos_mean"],
    koopman_shared["test"]["rollout_cos_mean_diffusion"],
    koopman_shared["test"]["rollout_cos_mean_advection"],
]
if "diffusion" in koopman_by_eq:
    bar_names.append("diff-only")
    bar_vals.append(koopman_by_eq["diffusion"]["test"]["rollout_cos_mean"])
if "advection" in koopman_by_eq:
    bar_names.append("adv-only")
    bar_vals.append(koopman_by_eq["advection"]["test"]["rollout_cos_mean"])
